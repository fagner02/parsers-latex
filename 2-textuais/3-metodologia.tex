...
O projeto será construído usando o \textit{framework Svelte}, sem \gls{ssr}, para que seja possível o funcionamento \textit{offline} da ferramentas, já que a ferramenta não teria acesso ao servidor não seria possível usá-lo para renderizar elementos. \textit{Svelte} compila a base de código e cria uma coleção de arquivos estáticos que constituem a página \textit{web} e a base para o suporte multi-plataforma da aplicação. 
...
O \textit{framework Tauri} constrói instaladores para \textit{desktop} diretamente da coleção de arquivos estáticos. A plataforma alvo dos instaladores é a plataforma na qual eles são construídos, já que o \textit{framework} não tem suporte para construção \textit{cross-platform} é necessária a utilização de uma maquiná virtual para construir instaladores para diferentes plataformas \textit{desktop}.
...
\begin{figure}[ht]
  \centering
  \captionsetup{width=16cm}
  \caption{Arquitetura da aplicação local}
  \label{fig:arqlocal}
  \tcbox[left=0cm, right=0cm, top=0cm, bottom=0cm,center]{\includegraphics[width=15.6cm]{figuras/local.png}}
  \Fonte{fornecida pelo próprio autor}
\end{figure}
\FloatBarrier
...
$\backslash$ & GET & redirecionar para página\\
$\backslash$login & POST & inicia uma conexão com a plataforma\\
$\backslash$register & POST & registrar uma nova plataforma\\
\end{NiceTabular}}
