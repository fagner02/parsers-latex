...
A análise dos \textit{heatmaps} de cliques revelou padrões de interação distintos entre as abas. Na Tarefa, observou-se distribuição esparsa das interações pelos campos de entrada como mostra a Figura \ref{fig:heatmap}. Nas abas algorítmicas, porém, identificou-se concentração em elementos visuais específicos, a tabela de \textit{parsing} na \gls{ll}(1), o autômato \gls{lr}(0) no \gls{slr}, e a árvore sintática na análise de \textit{strings}.
