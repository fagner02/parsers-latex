...
Este Capítulo apresenta os resultados obtidos na avaliação da ferramenta VANSI. A análise integra dados quantitativos de interação coletados automaticamente pela ferramenta com respostas qualitativas dos questionários de \textit{feedback}. A estrutura deste Capítulo está disposta de seguinte forma: a Seção \ref{sec:caracterizacao-participantes} fala sobre a caracterização dos dados coletados; a Seção \ref{sec:padroes-interacao} fala sobre a análise dos padrões de interação; a Seção \ref{sec:usabilidade} fala sobre a avaliação de usabilidade; a Seção \ref{sec:pedagogica} fala sobre a eficácia pedagógica; e, por fim, a Seção \ref{sec:avaliacao} fala sobre a avaliação geral.
